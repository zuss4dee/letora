...
export function SignContractForm({ token }: { token: string }) {
  ...
  if (signed) {
    return (
      <div className="border-t border-[rgb(72_72_72_/0.12)] bg-background dark:bg-[#0e0e0e]/30 px-6 py-8 text-center sm:px-8">
  ...
  return (
    <div className="border-t border-[rgb(72_72_72_/0.12)] bg-background dark:bg-[#0e0e0e]/25 px-6 py-6 sm:px-8">
      <label className="flex cursor-pointer items-start gap-3">
        <input
          type="checkbox"
          checked={agreed}
          onChange={(e) => setAgreed(e.target.checked)}
          className="mt-1 h-4 w-4 rounded border-[rgb(72_72_72_/0.35)] bg-background dark:bg-[#0e0e0e] accent-white"
        />
