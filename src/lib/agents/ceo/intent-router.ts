...
export function formatRouterHintForSystem(route: CEOIntentRoute): string {
  ...
  if (route.wantsOnboardingStateInspection) {
    lines.push(
      "- **Required (onboarding next-step — narrow reply):** Read-first: **resolve_onboarding_navigation** (and **get_contracts** / **prepare_referencing** only when the router listed them). In **visible chat**, stay short: one line **where they are** (stage/status from JSON), one line **what’s next** (single concrete action from **pending_task_names** / **ceo_resume_hint** / blocker fields), optional **one** bullet if a field is blocking. Do **not** narrate a full onboarding playbook, “everything is set up”, or generic product reassurance. Do **not** list steps that are **not** in the JSON. Settings / agency email **only** if **prepare_referencing** shows **missing_agency_email** (or equivalent) for **this** tenancy. No **draft_contract** or **chase_rent** unless JSON shows that gate. Approval-gated → **/dashboard/approvals**. Do **not** call **start_tenant_onboarding** or **create_tenant_and_tenancy** unless JSON proves no path exists and the user asks to proceed.",
    )
  }
