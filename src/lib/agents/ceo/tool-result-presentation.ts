...
const TOOL_SUMMARY_HINTS: Record<CEOToolName, string> = {
  ...
  draft_contract:
    "Read **saved**, **contract_id**, **error**, and **code**. If **code** is **llm_error** or **save_failed**, repeat the **error** string in plain language — do **not** blame “tenant record resolution”, “backend configuration”, “synchronization”, or “sync” unless those exact words appear in **error**. If **code** is **ambiguous_tenancy**, **multiple_tenants**, or **tenant_name_mismatch**, list **candidates** and ask the user to pick a person — do **not** say the tenancy does not exist at that address. If **code** is **ambiguous_property**, list property **candidates**. If **code** is **property_not_found**, suggest **list_tenants** or a slightly different address fragment — never claim a platform-wide backend failure. If **saved** is true, confirm the draft was saved and point to **/dashboard/contracts** (same as manual “New contract”). **tenancy_id** may be omitted when the draft matched tenant + property without a separate tenancy row, like the dashboard. Do not paste the full contract unless asked.",
