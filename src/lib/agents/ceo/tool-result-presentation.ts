...
const TOOL_SUMMARY_HINTS: Record<CEOToolName, string> = {
  ...
  resolve_onboarding_navigation:
    "For next-step onboarding questions, use this read-first shape: **current stage** → **completed steps** → **blocker (if any)** → **next valid onboarding action**. Use **only** fields present in this JSON (**pending_task_names**, **referencing_complete**, **tasks_complete**/**tasks_total**, **onboarding_status**, etc.) — do **not** invent tasks or generic Settings steps. Do **not** discuss **chase_rent** or **tenancy agreement** drafting unless this JSON (or **get_contracts** in the same turn) shows that as the next gate. Distinguish human gates from automation: if the next step is approval-gated, say the landlord must approve in **/dashboard/approvals**. Mention Settings / agency email **only** when JSON shows **missing_agency_email** or an equivalent explicit blocker. Avoid wording that implies they must manually work through every checklist item. Prefer: “Open onboarding to view status.” If **referencing_complete** is true, do not claim referencing is pending. Offer execution only after the landlord explicitly says proceed. Give the **Open onboarding** button (UI). If needs_tenant or multiple_tenants, ask which tenant or use chips. If multiple_tenancies, ask them to pick the right property. Do not paste raw JSON.",
}
