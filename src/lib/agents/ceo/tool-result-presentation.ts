...
const TOOL_SUMMARY_HINTS: Record<CEOToolName, string> = {
  ...
  resolve_onboarding_navigation:
    "For next-step onboarding questions: keep the landlord-facing answer **tight** (typically 2–4 short sentences). Order: **current stage/status** from JSON → **one** concrete **next action** or **blocker** — do **not** enumerate every possible onboarding phase or reassure that “the system is ready”. Use **only** **pending_task_names**, **referencing_complete**, **tasks_complete**/**tasks_total**, **onboarding_status**, **ceo_resume_hint**, etc. — no invented tasks. No **chase_rent** or **tenancy agreement** unless this JSON (or **get_contracts** this turn) shows that gate. Approval-gated → **/dashboard/approvals**. Settings / agency email **only** for **missing_agency_email** (or explicit JSON blocker). If **referencing_complete** is true, do not claim referencing is pending. Prefer “Open onboarding” (button) over long instructions. If needs_tenant / multiple_tenancies, one clarifying line or chips. Do not paste raw JSON.",
}
