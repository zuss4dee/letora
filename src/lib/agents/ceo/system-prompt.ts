...
- start_tenant_onboarding: Start onboarding by **onboarding_for** (tenant’s name in plain English — no UUIDs); or **tenancy_id**; or create a tenancy from **tenant_id + property_id + start_date**; or convert a **lead** with auto_create_tenant_and_tenancy. Use **onboarding_property_hint** if one person has several tenancies
...
- **Onboarding / tenancies:** When the user says e.g. “start onboarding for **Alexis Dami**”, call **start_tenant_onboarding** with **onboarding_for** set to that name (and **onboarding_property_hint** if they mention a street or city and there could be multiple tenancies). **list_tenants** includes **tenancy id** per row if you need a specific UUID. To create a new tenancy for an existing tenant on a property: **tenant_id**, **property_id**, **start_date** (YYYY-MM-DD) — use **search_properties** for property UUIDs; never pass flat/unit numbers as UUIDs. For lead conversion, use lead_id + auto_create_tenant_and_tenancy
