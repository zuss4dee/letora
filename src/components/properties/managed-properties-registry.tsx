...
export function ManagedPropertiesRegistry({
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-[#0e0e0e]">
      {/* OPERATIONAL HEADER */}
      <div className="flex shrink-0 items-center justify-between border-b border-[#232323] bg-[#0e0e0e] px-6 py-4">
        <div>
          <h1 className="text-[16px] font-bold tracking-tight text-white uppercase">Portfolio Operations</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <AddPropertyDialog
            trigger={
              <button
                type="button"
                className="flex h-8 items-center gap-1.5 border border-[#232323] bg-[#111111] px-3 text-[10px] font-bold uppercase tracking-wider text-zinc-400 hover:border-zinc-600 hover:text-white transition-colors"
              >
      ...
      {/* KPI SUMMARY STRIP */}
      <div className="grid shrink-0 grid-cols-5 border-b border-[#232323] bg-[#0B0B0B]">
        {[
          ...
          <div key={i} className="flex flex-col border-r border-[#232323] p-4 last:border-r-0">
            ...
            <span className={cn(
              ...
              stat.tone === 'emerald' ? "text-emerald-400" : "text-white"
            )}>
              {stat.value}
            </span>
          </div>
      ...
      <main className="flex min-h-0 flex-1 overflow-hidden">
        <section className={cn(
          "flex min-h-0 min-w-0 flex-1 flex-col bg-[#111111]",
          inspectorOpen && "border-r border-[#232323]"
        )}>
          <div className="sticky top-0 z-10 grid grid-cols-12 border-b border-[#232323] bg-[#111111] px-6 py-2 text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-500">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto divide-y divide-[#232323]">
            {rows.length === 0 ? (
              ...
              filteredRows.map((row) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "grid cursor-pointer grid-cols-12 items-center gap-4 px-6 py-4 transition-colors hover:bg-[#161616]",
                      isSelected ? "bg-[#111111] shadow-[inset_2px_0_0_0_#ffffff]" : "bg-transparent"
                    )}
                  >
                    <div className="col-span-5 min-w-0">
                      <div className="flex flex-col">
                        <span className="text-[12px] font-semibold text-zinc-100">{row.identityTitle}</span>
                    ...
                    <div className="col-span-2 flex flex-col items-center justify-center">
                      ...
                      <div className="mt-1 h-1 w-12 bg-[#232323]">
                        <div
                    ...
                    <div className="col-span-2 text-right">
                      <div className={cn(
                        ...
                        row.rentOverdueGbp > 0 ? "text-rose-400" : "text-zinc-100"
                      )}>
                    ...
                    <div className="col-span-2 flex flex-col items-center justify-center">
                      <span className={cn(
                        ...
                        row.openMaintenanceCount > 0 ? "text-amber-400" : "text-zinc-500"
                      )}>
          ...
          {inspectorOpen && selected ? (
            <aside className="hidden h-full min-h-0 w-96 shrink-0 flex-col border-l border-[#232323] bg-[#0B0B0B] xl:flex">
              <div className="shrink-0 border-b border-[#232323] bg-[#0e0e0e] p-6">
                <div className="mb-4 flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">Asset Profile</span>
                  <button
                    onClick={() => setSelectedPropertyId(null)}
                    className="group flex h-6 w-6 items-center justify-center border border-[#232323] hover:border-zinc-500 transition-colors"
                  >
                    <X className="size-3.5 text-zinc-500 group-hover:text-white" />
                  </button>
                </div>
                <h2 className="text-[20px] font-bold leading-tight text-white">{selected.identityTitle}</h2>
                <p className="mt-1 text-[11px] uppercase tracking-tight text-zinc-500">{selected.identitySubline}</p>
              </div>

              <div className="min-h-0 flex-1 overflow-y-auto">
                <div className="grid grid-cols-2 gap-px border-b border-[#232323] bg-[#232323]">
                  <div className="bg-[#0B0B0B] p-4">
                    <p className="mb-1 text-[9px] font-bold uppercase tracking-widest text-zinc-500">Occupancy</p>
                    <p className={cn(
                      "text-xl font-bold tabular-nums",
                      selected.occupancyPct === 100 ? "text-emerald-400" : 
                      selected.occupancyPct > 0 ? "text-amber-400" : "text-zinc-500"
                    )}>
                      {selected.occupancyPct}%
                    </p>
                  </div>
                  <div className="bg-[#0B0B0B] p-4 text-right">
                    <p className="mb-1 text-[9px] font-bold uppercase tracking-widest text-zinc-500">Arrears Risk</p>
                    <p className={cn(
                      "text-xl font-bold tabular-nums",
                      selected.rentOverdueGbp > 0 ? "text-rose-400" : "text-white"
                    )}>
                  ...
                  <div className="bg-[#0B0B0B] p-4">
                    ...
                    <p className={cn(
                      "text-xl font-bold tabular-nums",
                      selected.openMaintenanceCount > 0 ? "text-amber-400" : "text-white"
                    )}>
                ...
                {selected.currentTenantName && (
                  <div className="border-b border-[#232323] bg-[#0e0e0e] p-6 text-center">
                    <p className="mb-2 text-[9px] font-bold uppercase tracking-widest text-zinc-500">Primary Occupant</p>
                    <p className="text-[16px] font-bold text-white">{selected.currentTenantName}</p>
                    <Link
                ...
                <div className="p-6">
                  ...
                  {inspectorActivityEntries.length === 0 ? (
                    <div className="border border-dashed border-[#232323] p-4 text-center">
                    ...
                    <div className="space-y-4">
                      {visibleActivityEntries.map((entry) => (
                        <div key={entry.id} className="relative pl-4">
                          ...
                          <div className="flex flex-col gap-0.5">
                            <span className="text-[10px] font-bold text-zinc-200">{entry.title}</span>
                      ...
                      {activityLogHasMore && (
                        <button
                          ...
                          className="w-full border border-[#232323] py-2 text-[9px] font-bold uppercase tracking-widest text-zinc-500 hover:border-zinc-700 hover:text-zinc-300 transition-colors"
                        >
              ...
              <div className="mt-auto border-t border-[#232323] bg-[#0e0e0e] p-4 space-y-2">
                {[
                  ...
                  <Link
                    key={i}
                    href={action.href}
                    className="flex items-center justify-between border border-[#232323] bg-[#111111] px-3 py-2 text-[10px] font-bold uppercase text-zinc-400 hover:border-zinc-600 hover:text-white transition-colors"
                  >
                    {action.label}
