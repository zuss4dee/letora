...
type Props = {
  propertyId: string;
  initial: AddPropertyInput;
  triggerLabel?: string;
};

export function EditPropertyDialog({ propertyId, initial, triggerLabel = "Edit property" }: Props) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);

  const defaultValues = useMemo<AddPropertyInput>(() => initial, [initial]);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className={DIALOG_SINGLE_COLUMN_CLASS}>
        <DialogHeader>
          ...
          <DialogDescription>Update this property&apos;s details.</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className={DIALOG_FORM_STACK_CLASS}>
            ...
            <div className="grid w-full min-w-0 gap-4 sm:grid-cols-2">
              <div className={DIALOG_FIELD_CLASS}>
                <Label htmlFor="ep-beds">Bedrooms</Label>
                <Input
                  id="ep-beds"
                  className="w-full"
                  type="number"
              ...
              <div className={DIALOG_FIELD_CLASS}>
                <Label htmlFor="ep-baths">Bathrooms</Label>
                <Input
                  id="ep-baths"
                  className="w-full"
                  type="number"
            ...
            <div className={DIALOG_FIELD_CLASS}>
              <Label htmlFor="ep-rent">Monthly rent (£)</Label>
              <Input
                id="ep-rent"
                className="w-full"
                type="number"
