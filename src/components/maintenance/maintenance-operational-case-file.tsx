...
function TriageChip({ category }: { category: string | null }) {
  if (!category?.trim()) {
    return (
      <span className="border border-border dark:border-[#333333] bg-background dark:bg-[#0B0B0B] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-zinc-400">
        Analysing…
      </span>
    );
  }
  ...
  if (c === "routine") {
    return (
      <span className="border border-border dark:border-[#333333] bg-background dark:bg-[#141414] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-zinc-400">
        routine
      </span>
    );
  }
...
function Section({
  ...
  return (
    <section
      id={id}
      className="scroll-mt-28 border border-border dark:border-[#333333] bg-background dark:bg-[#161616]"
    >
      <header className="border-b border-border dark:border-[#282828] bg-background dark:bg-[#161616] px-5 py-4 md:px-6">
        <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-500">{eyebrow}</p>
        <h3 className="mt-1 text-xs font-bold uppercase tracking-wider text-zinc-900 dark:text-white">{title}</h3>
        {subtitle ? (
    ...
    <div className="@container/main flex min-h-0 flex-1 flex-col bg-background font-['Inter',system-ui,sans-serif] text-zinc-900 dark:bg-[#0B0B0B] dark:text-[#e5e2e1]">
      ...
      <div className="shrink-0 border-b border-border dark:border-[#282828] bg-background dark:bg-[#141414] px-4 py-3 md:px-6">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
          <span className="size-1.5 shrink-0 bg-background dark:bg-[#afefdd]" aria-hidden />
          <Link
            href={backHref}
            className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-[#e5e2e1] underline-offset-4 transition-colors hover:text-zinc-900 dark:hover:text-zinc-900 dark:text-white hover:underline"
          >
            ← Maintenance Center
          </Link>
          <span className="hidden h-3 w-px bg-background dark:bg-[#333333] sm:block" aria-hidden />
      ...
      {/* Case header */}
      <header className="shrink-0 border-b border-border dark:border-[#282828] bg-background dark:bg-[#0B0B0B] px-4 py-6 md:px-6 md:py-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 flex-1 space-y-3">
            <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-zinc-500">
              Full case workspace
            </p>
            <h1 className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-white md:text-2xl">{titleLine}</h1>
          ...
          <div className="flex shrink-0 flex-col items-stretch gap-3 sm:flex-row sm:items-center lg:flex-col lg:items-end">
            <Link
              ...
              className="border border-border dark:border-[#333333] px-4 py-2 text-center font-mono text-[10px] font-bold uppercase tracking-wider text-zinc-400 transition-colors hover:border-zinc-600 hover:text-white sm:min-w-[12rem]"
            >
              Approvals queue
            </Link>
            {canResolve ? (
      ...
      <div className="grid shrink-0 grid-cols-2 gap-px border-b border-border dark:border-[#282828] bg-background dark:bg-[#282828] md:grid-cols-4">
        {[
          ...
          <div key={k.label} className="bg-background dark:bg-[#0B0B0B] px-4 py-3 md:py-4">
            <p className="mb-1 text-[9px] font-bold uppercase tracking-wider text-zinc-500">{k.label}</p>
            <p className="text-[11px] font-semibold tabular-nums text-zinc-900 dark:text-white">{k.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row lg:divide-x lg:divide-[#282828]">
        {/* Main dossier */}
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 pb-24 md:p-6 md:pb-28 lg:border-r lg:border-border dark:border-[#282828]">
          <Section
            id="request-issue"
            eyebrow="Incident"
          ...
          >
            ...
            <div className="space-y-3 border-t border-border dark:border-[#282828] pt-4">
              <FieldRow
                label="Summary"
            ...
            {detail.maintenanceAgentRunId ? (
              <p className="border-t border-border dark:border-[#282828] pt-4 font-mono text-[10px] uppercase tracking-[0.1em] text-zinc-600">
          ...
          <section className="border border-border dark:border-[#333333] bg-background dark:bg-[#161616]">
            <header className="border-b border-border dark:border-[#282828] bg-background dark:bg-[#161616] px-5 py-4 md:px-6">
              <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-500">Comms ledger</p>
              <h3 className="mt-1 text-xs font-bold uppercase tracking-wider text-zinc-900 dark:text-white">Related outbound emails</h3>
            ...
            <div className="border-t border-border dark:border-[#282828]">
        ...
        {/* Timeline sidebar */}
        <aside className="shrink-0 border-t border-border dark:border-[#282828] bg-background dark:bg-[#0B0B0B] lg:w-72 lg:border-t-0 lg:border-l lg:border-border dark:border-[#282828]">
          <div className="sticky top-0 space-y-0 border-border dark:border-[#282828] p-4 md:p-6">
