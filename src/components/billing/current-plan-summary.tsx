...
export function CurrentPlanSummary({
  ...
  const amountLine = paying ? getSubscriptionAmountLine(settings?.subscriptionPlan ?? null) : null;
    ...
    >
        ...
        <div className="mt-4 rounded-lg border border-zinc-800/50 bg-muted/30 px-3 py-3">
          ...
          {amountLine ? (
            <p className="mt-2 text-sm text-muted-foreground">
              After the trial, Starter continues at <span className="tabular-nums text-foreground">{amountLine}</span>{" "}
              unless you cancel in {billingProvider}.
            </p>
          ) : null}
        </div>
      ) : null}
        ...
        <div className="mt-4 space-y-1">
          ...
          {periodEnd ? (
            <p className="text-sm text-muted-foreground">
              Current billing period ends <span className="tabular-nums text-foreground">{periodEnd}</span>.
            </p>
          ) : null}
        </div>
      ) : null}
      ...
      {statusLc === "past_due" ? (
        <p className="mt-4 text-sm text-amber-600 dark:text-amber-400">
          Payment past due — update your card in the {billingProvider} customer portal (under Billing) so Starter stays
          active.
        </p>
      ) : null}

      {(statusLc === "inactive" || statusLc === "canceled" || statusLc === "cancelled") &&
      (hasStripeCustomer || hasPolarCustomer) ? (
        <p className="mt-4 text-sm text-muted-foreground">
          This subscription is no longer active. You can start again from{" "}
          <Link href="/dashboard/billing" className="font-medium text-foreground underline-offset-4 hover:text-zinc-400 hover:underline">
            Billing
          </Link>
          .
        </p>
      ...
      {catalog?.tagline ? (
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          You&apos;re on a paid Starter workspace. Details sync from {billingProvider} within a minute after checkout.
        </p>
      ) : null}
      ...
      {!paying && !(hasStripeCustomer || hasPolarCustomer) ? (
        <p className="mt-4 text-sm text-muted-foreground">
          ...
          <span className="tabular-nums text-foreground">{FREE_WORKSPACE_PROPERTY_CAP}</span> properties). Subscribe from{" "}
          <Link href="/dashboard/billing" className="font-medium text-foreground underline-offset-4 hover:text-zinc-400 hover:underline">
            Billing
          </Link>{" "}
          for unlimited properties and agents.
        </p>
      ) : null}
