...
export function SafetyAlertsCard({ alerts }: { alerts: SafetyAlertRow[] }) {
  useDashboardPollRefresh();
  if (alerts.length === 0) return null;

  return (
    <Card className="border-red-300 bg-red-50/80 dark:border-red-900/50 dark:bg-red-950/30">
      <CardHeader className="border-b border-red-200 dark:border-red-900/40">
        ...
        <p className="text-sm font-normal text-red-800/90 dark:text-red-200/90">
      ...
      <CardContent className="space-y-3 pt-4">
        {alerts.map((a) => {
          ...
          return (
            ...
            >
              <p className="font-medium text-red-950 dark:text-red-50">{alertTitle(payload)}</p>
              ...
              {mid ? (
                <Link
                  ...
                  className="mt-2 inline-block text-sm font-medium text-red-800 underline-offset-4 hover:underline dark:text-red-300"
                >
                  View request
                </Link>
              ) : null}
