...
function LateRentArrearsRibbon({ row }: { row: ArrearsQueueRow }) {
  ...
  > = {
    chase_pending: {
      label: "CHASE PENDING",
      className: "border border-amber-600/40 bg-amber-950/40 text-amber-200",
    ...
    chased: {
      label: "CHASED",
      className: "border border-emerald-800/40 bg-emerald-950/30 text-emerald-300",
      wrapWithApprovalsLink: false,
    },
    no_action: {
      label: "NO ACTION",
...
export function CommandCenterArrearsQueueList({ rows }: { rows: ArrearsQueueRow[] }) {
  ...
  return (
    <>
      <div className="divide-y divide-[#282828]">
        {visible.map((row) => (
          <div
            ...
            className="group flex min-h-[4.5rem] items-center justify-between gap-3 p-4 transition-colors hover:bg-[#242424]"
          >
            <Link
            ...
            >
              ...
              <div className="min-w-0">
                <p className="truncate text-[13px] font-bold text-white">{row.tenantName}</p>
                <p className="truncate font-mono text-[10px] uppercase text-zinc-500">{row.propertyAddress}</p>
                <p className="mt-1 truncate font-mono text-[9px] uppercase tracking-wide text-zinc-600">
            ...
            <div className="flex shrink-0 items-center gap-4 sm:gap-6">
              <div className="hidden text-right sm:block">
                <p className="text-[13px] font-bold tabular-nums text-white">{formatMoney(row.amountDue)}</p>
      ...
      {overflow && !showAll ? (
        <div className="border-t border-[#282828] p-3 text-center">
          <button
            type="button"
            onClick={() => setShowAll(true)}
            className="font-mono text-[10px] font-bold uppercase tracking-wider text-zinc-500 underline-offset-4 transition-colors hover:text-zinc-300"
          >
