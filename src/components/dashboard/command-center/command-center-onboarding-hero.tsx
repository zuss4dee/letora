        ...
        <div className="relative grid grid-cols-1 gap-12 lg:grid-cols-12">
          <div className="lg:col-span-7">
            <div className="mb-6 flex items-center gap-3">
              <div className="flex size-10 items-center justify-center border border-zinc-200 bg-zinc-100 dark:border-[#333333] dark:bg-[#0B0B0B]">
                <Sparkles className="size-5 text-zinc-900 dark:text-white" />
              </div>
            ...
            <h2 className="font-['Inter',sans-serif] text-3xl font-bold tracking-tight text-zinc-900 md:text-4xl lg:text-5xl dark:text-white">
              Your portfolio, <br />
              <span className="text-zinc-500 dark:text-zinc-500">managed by agents.</span>
            </h2>

            <p className="mt-6 max-w-xl text-sm leading-relaxed text-zinc-600 md:text-base dark:text-zinc-400">
              Letora is an agentic platform designed to handle the heavy lifting of landlord operations. Import your portfolio
              data, and our specialized agents will begin auditing leases, tracking rent, and managing maintenance automatically.
            </p>
          ...
          <div className="lg:col-span-5">
            <div className="space-y-6">
              <h3 className="font-mono text-[10px] font-bold uppercase tracking-widest text-zinc-500">Initial Deployment Steps</h3>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div
                    key={item.step}
                    className="flex items-start gap-4 border border-zinc-200 bg-zinc-50 p-4 dark:border-[#333333] dark:bg-[#0B0B0B]"
                  >
                    <span className="font-mono text-[10px] font-black text-zinc-500 dark:text-zinc-600">{item.step}</span>
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-900 dark:text-white">{item.label}</p>
                        <span
                          className={`font-mono text-[9px] ${item.status === "READY" ? "text-emerald-700 dark:text-[#afefdd]" : "text-zinc-500 dark:text-zinc-700"}`}
                        >
                          {item.status}
                        </span>
                      </div>
                      <p className="mt-1 text-[11px] text-zinc-600 dark:text-zinc-500">{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
